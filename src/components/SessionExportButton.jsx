...
import { exportSessionMarkdown, exportSessionPng } from '../utils/exporter';
...
export default function SessionExportButton() {
  ...
  const handleExportMarkdown = () => {
    exportSessionMarkdown(handleStatusChange);
  };

  const handleExportPng = () => {
    exportSessionPng(handleStatusChange);
  };

  const titleText = status || "导出整页会话为 Markdown";

  const btnProps = stylex.props(styles.button, hovered && styles.buttonHover);
  ...
  return (
    <>
      <button
        ...
        className={`chatgpt-export-session-md-btn ${btnProps.className || ''}`}
        style={btnProps.style}
      >
        {status ? (
          <span style={{ fontSize: '10px', padding: '0 4px', whiteSpace: 'nowrap' }}>{status}</span>
        ) : (
          EXPORT_ICON_SVG
        )}
      </button>
