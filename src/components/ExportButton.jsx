import React, { useRef, useState, useEffect } from "react";
import * as stylex from "@stylexjs/stylex";
import ExportDropdown from "./ExportDropdown";
import { exportMarkdown, exportMessagePng } from "../utils/exporter";

const styles = stylex.create({
  button: {
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
    width: "28px",
    height: "28px",
    padding: 0,
    border: "none",
    borderRadius: "6px",
    backgroundColor: "transparent",
    color: "var(--text-secondary)",
    cursor: "pointer",
    transition: "background-color 0.1s ease, color 0.1s ease",
  },
  buttonHover: {
    backgroundColor: "rgba(127, 127, 127, 0.12)",
    color: "var(--text-primary)",
  },
});

const EXPORT_ICON_SVG = (
  <svg
    viewBox='0 0 1024 1024'
    xmlns='http://www.w3.org/2000/svg'
    aria-hidden='true'
    style={{ width: "18px", height: "18px" }}
  >
    <path
      fill='currentColor'
      d='M160 213.333333C95.573333 213.333333 42.666667 266.24 42.666667 330.666667v362.666666C42.666667 757.76 95.573333 810.666667 160 810.666667h704c64.426667 0 117.333333-52.906667 117.333333-117.333334v-362.666666c0-64.426667-52.906667-117.333333-117.333333-117.333334h-704z m0 64h704c29.824 0 53.333333 23.509333 53.333333 53.333334v362.666666c0 29.824-23.509333 53.333333-53.333333 53.333334h-704A52.864 52.864 0 0 1 106.666667 693.333333v-362.666666C106.666667 300.842667 130.176 277.333333 160 277.333333z m88.917333 85.333334A56.896 56.896 0 0 0 192 419.584V618.666667a42.666667 42.666667 0 1 0 85.333333 0v-155.413334l54.698667 74.090667a42.666667 42.666667 0 0 0 68.544 0.149333L448 473.92V618.666667a42.666667 42.666667 0 1 0 85.333333 0v-199.082667a56.896 56.896 0 0 0-104.32-31.466667l-62.762666 99.306667-69.76-101.184A56.96 56.96 0 0 0 250.368 362.666667h-1.450667zM725.333333 362.666667a42.666667 42.666667 0 0 0-42.666666 42.666666v106.666667h-31.253334c-27.797333 0-42.730667 32.618667-24.661333 53.717333l71.125333 82.986667a36.181333 36.181333 0 0 0 54.912 0l71.125334-82.986667c18.090667-21.12 3.136-53.717333-24.661334-53.717333H768v-106.666667a42.666667 42.666667 0 0 0-42.666667-42.666666z'
    />
  </svg>
);

export default function ExportButton({ actionBar }) {
  const buttonRef = useRef(null);
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [dropdownRect, setDropdownRect] = useState(null);
  const [status, setStatus] = useState(null); // Feedback like "已导出"
  const [hovered, setHovered] = useState(false);

  const handleButtonClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (buttonRef.current) {
      setDropdownRect(buttonRef.current.getBoundingClientRect());
      setDropdownOpen((prev) => !prev);
    }
  };

  const handleStatusChange = (newStatus) => {
    setStatus(newStatus);
  };

  // Reset status text after 1200ms
  useEffect(() => {
    if (status && status !== "正在生成...") {
      const timer = setTimeout(() => {
        setStatus(null);
      }, 1200);
      return () => clearTimeout(timer);
    }
  }, [status]);

  const handleExportMarkdown = () => {
    exportMarkdown(actionBar, handleStatusChange);
  };

  const handleExportPng = () => {
    const turnNode =
      actionBar.closest("[data-message-author-role]") ||
      actionBar.closest("article") ||
      actionBar.closest('div[data-testid*="turn"]') ||
      actionBar.closest(".group\\/conversation-turn") ||
      actionBar.parentElement?.parentElement ||
      actionBar.parentElement;
    exportMessagePng(turnNode, handleStatusChange);
  };

  const titleText = status || "导出为 Markdown";

  const btnProps = stylex.props(styles.button, hovered && styles.buttonHover);

  return (
    <>
      <button
        ref={buttonRef}
        type='button'
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        onClick={handleButtonClick}
        title={titleText}
        aria-label={titleText}
        className={`chatgpt-export-md-btn ${btnProps.className || ""}`}
        style={btnProps.style}
      >
        {status ? (
          <span style={{ fontSize: "10px", padding: "0 4px", whiteSpace: "nowrap" }}>{status}</span>
        ) : (
          EXPORT_ICON_SVG
        )}
      </button>

      {dropdownOpen && dropdownRect && (
        <ExportDropdown
          triggerRect={dropdownRect}
          onClose={() => setDropdownOpen(false)}
          onExportMarkdown={handleExportMarkdown}
          onExportPng={handleExportPng}
        />
      )}
    </>
  );
}
